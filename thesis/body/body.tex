...
\chapter{手法}
\label{sec:orgd61d606}
\section{開発手法}
\label{sec:orgfd0e6e0}
//XSSとか引用をいれる. そして何なのか調べる
...
\begin{figure}[htbp]
\centering
\includegraphics[width=15cm]{./fig/MTV.png}
\caption{Django MTV構造に基づくリクエスト処理の流れ.}
\end{figure}
...
\chapter{結果と考察}
\label{sec:org09a7541}
\section{アプリ機能}
\label{sec:org1b6706b}
...
\subsection{旅行作成画面}
\label{sec:org0d61ead}

図3.5に示す旅行作成画面では, 旅行そのものを作成し, 計画している旅行のチェックリストなど全てのデータが集約される. 
...
\subsection{旅行詳細画面}
\label{sec:orgbaa4867}

旅行作成時に入力された期間により, 旅行の日数分だけタブを動的に生成する機能を実装した. 
また, 旅行に含まれる観光場所を「全体」「1日目」「2日目」など日程ごとに切り替えて表示し, ユーザーはタブを操作することで旅行全体の流れを把握できるようになる. 
各地点には名称や住所, 地点間の所要時間が表示されている. 
それぞれの日程に追加された地点は右側の地図上に表示され, 来訪場所と訪問順序が視覚化されるように実装した. 
また, ユーザーは各日程の地点の削除や訪問順序の並び替えができ, それに応じて地点間の所要時間や地図上の表示も更新される. 
これにより, 異なる日程の操作を誤って行ってしまうことや, 非現実的なスケジュールや移動の偏りにも気づきやすくなる. 

図3.8のように地図上に地点を表示するためには, まずGoogle Maps JavaScript APIを読み込む必要がある. 
本システムでは, 

// ここがPDF上でちゃんと表示されない

<script src="\url{https://maps.googleapis.com/maps/api/js?key}=\{\{ google\_maps\_api\_key \}\}\&callback=initMap" async defer></script>
...
図3.9のよにタブの切り替えに連動して, 地図上に表示される観光地もその日に対応するもののみに自動で更新されるため, 
...
onStart: function(evt) {
	      ...
	      const durationElements = activeTabPane ? activeTabPane.querySelectorAll('.travel-duration') : [];
...
これらはMapboxのDirections API[5]を活用して開発を行っている. 以下にその処理の流れを示す. 
...
\label{sec:org1c630db}

地点登録機能では, MapboxのSearch Box API[6]を活用している. 

施設名の入力画面は図3.11である. 
これにより,ユーザーは住所だけでなく, 観光地名やカテゴリ, 通り名などを入力することで, 目的の場所を効率的に検索できる. 
...
精度面ではGoogle Maps APIの方がOpenStreetMapやMapboxには勝っている一方, Google Maps APIはコスト的なハードルがある. (詳しく説明したい)しかし, MapboxのSearch Box APIは月10万回まで無料で利用できるためコスト面でも導入しやすいという利点がある.
\begin{figure}[htbp]
\centering
...
この処理は Django のシグナル機能[7]を用いて実現しており, 以下のような関数を定義している. 
本実装では, シグナル機能を利用することで, ユーザーが新しく登録された際に, 初期チェックリストをコピーする処理を自動で実行できるようになった.

\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./fig/receive.png}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./fig/created.png}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./fig/items.png}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=12cm]{./fig/user_items.png}
\end{figure}
...
\label{sec:org4d99b5d}
本システムでは, Bootstrap 5.1.3[8]を採用し, ユーザーインターフェースの視覚的理解を促進した. 
...
さらにFont Awesome[9]を併用し, ボタンにアイコンを付与することで, 言語に依存しない直感的な操作を可能にした. 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=17cm]{./fig/css.png}
\end{figure}
