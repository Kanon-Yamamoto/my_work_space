...
\item セキュリティ対策は,フレームワーク自体に標準で安全性を確保する仕組みが備えられている.
\end{enumerate}
また, 実装にはHTML, JavaScript, CSSを使用し,UIライブラリとしてはBootstrapを採用した.

\section{結果}
\label{sec:org4c60ab1}
...
旅行詳細画面では, 地図表示に Google Maps JavaScript API を使用した. 観光地の登録には Mapbox Search Box API を用い, 移動時間の自動算出には Mapbox Directions API を活用している. また, 訪問順序の入れ替え機能については SortableJS を用いて実装した.
...
旅行詳細が完成後, 生成したリンクを家族や友人に共有することで, 受け手は旅行情報を閲覧できる.
\begin{figure}[htbp]
\centering
